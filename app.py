...
# ── Токен бота
st.markdown('<div class="section-card"><div class="section-label">Токен Telegram бота</div>', unsafe_allow_html=True)
token = st.text_input("", placeholder="1234567890:AAF...", type="password", label_visibility="collapsed", key="token_input")
...
# ── ID администратора
st.markdown('<div class="section-card"><div class="section-label">Telegram ID администратора (получит отчёт)</div>', unsafe_allow_html=True)
admin_id = st.text_input("", placeholder="5980876264", label_visibility="collapsed", key="admin_input")
st.markdown('</div>', unsafe_allow_html=True)

# ── Кнопка запуска
can_send = bool(uploaded_file and token and admin_id)
